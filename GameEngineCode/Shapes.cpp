...
//Additional Code
//Compound shape is a shape made up of four rectangles, this is specificly made for the 
//rooms hitbox but could be expanded to create more complex hitboxes
CompoundShape2D::CompoundShape2D()
{

}
//sets the four rectangles that will make up a 4 rectangle compound shape
void CompoundShape2D::AddRectangle(Rectangle2D north, Rectangle2D east, Rectangle2D south, Rectangle2D west)
{
	rectangles[0] = north;
	rectangles[1] = east;
	rectangles[2] = south;
...
//Check if any of the four rectangles are intersecting with other objects
bool CompoundShape2D::Intersects(const IShape2D& other) const
{
		for (Rectangle2D nextRectangle : rectangles)
		{
			if (nextRectangle.Intersects(other))
				return true;
		}
		return false;
}
