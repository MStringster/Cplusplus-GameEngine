...
//Compound Shape class to create a compound of four Rectangles
class CompoundShape2D : public IShape2D
{
protected:
	Rectangle2D rectangles[4];

public:
	CompoundShape2D();
	//Add a rectangle to the compound shape
	void AddRectangle(Rectangle2D north, Rectangle2D East, Rectangle2D south, Rectangle2D west);
	//Retruns true if the compound shape is intersecting other shapes
	bool Intersects(const IShape2D& other) const;
};